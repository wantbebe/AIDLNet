...
def window_partition(x, window_size):
    ...
    Args:
        x (torch.Tensor): Input tensor with shape (B, H, W, C_glsa).
        window_size (int): Size of each window.

    Returns:
        (Tuple[torch.Tensor, Tuple[int, int]]): A tuple containing:
            - windows (torch.Tensor): Partitioned windows with shape (B * num_windows, window_size, window_size, C_glsa).
...
def window_unpartition(windows, window_size, pad_hw, hw):
    ...
    Args:
        windows (torch.Tensor): Input tensor of windowed sequences with shape (B * num_windows, window_size,
            window_size, C_glsa), where B is the batch size, num_windows is the number of windows, window_size is
            the size of each window, and C_glsa is the number of channels.
        window_size (int): Size of each window.
        pad_hw (Tuple[int, int]): Padded height and width (Hp, Wp) of the input before windowing.
        hw (Tuple[int, int]): Original height and width (H, W) of the input before padding and windowing.

    Returns:
        (torch.Tensor): Unpartitioned sequences with shape (B, H, W, C_glsa), where B is the batch size, H and W
            are the original height and width, and C_glsa is the number of channels.
...
def get_rel_pos(q_size: int, k_size: int, rel_pos: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        ...
        rel_pos (torch.Tensor): Relative position embeddings with shape (L, C_glsa), where L is the maximum relative
            distance and C_glsa is the embedding dimension.
...
def add_decomposed_rel_pos(
    ...
    Args:
        ...
        q (torch.Tensor): Query tensor in the attention layer with shape (B, q_h * q_w, C_glsa).
        rel_pos_h (torch.Tensor): Relative position embeddings for height axis with shape (Lh, C_glsa).
        rel_pos_w (torch.Tensor): Relative position embeddings for width axis with shape (Lw, C_glsa).
    ...
    Examples:
        >>> B, C_glsa, q_h, q_w, k_h, k_w = 1, 64, 8, 8, 8, 8
        >>> attn = torch.rand(B, q_h * q_w, k_h * k_w)
        >>> q = torch.rand(B, q_h * q_w, C_glsa)
        >>> rel_pos_h = torch.rand(2 * max(q_h, k_h) - 1, C_glsa)
        >>> rel_pos_w = torch.rand(2 * max(q_w, k_w) - 1, C_glsa)
