...
class CXBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.dwconv(x)
        x = self.norm(x)
        x = x.permute(0, 2, 3, 1)  # (N, C_glsa, H, W) -> (N, H, W, C_glsa)
        x = self.pwconv1(x)
        x = self.act(x)
        x = self.pwconv2(x)
        if self.gamma is not None:
            x = self.gamma * x
        x = x.permute(0, 3, 1, 2)  # (N, H, W, C_glsa) -> (N, C_glsa, H, W)
...
def do_pool(x: torch.Tensor, pool: nn.Module, norm: nn.Module = None) -> torch.Tensor:
    ...
    if pool is None:
        return x
    # (B, H, W, C_glsa) -> (B, C_glsa, H, W)
    x = x.permute(0, 3, 1, 2)
    x = pool(x)
    # (B, C_glsa, H', W') -> (B, H', W', C_glsa)
    x = x.permute(0, 2, 3, 1)
    if norm:
        x = norm(x)
...
class MultiScaleAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        B, H, W, _ = x.shape
        # qkv with shape (B, H * W, 3, nHead, C_glsa)
        qkv = self.qkv(x).reshape(B, H * W, 3, self.num_heads, -1)
        # q, k, v with shape (B, H * W, nheads, C_glsa)
        ...
        # Torch's SDPA expects [B, nheads, H*W, C_glsa] so we transpose
...
class PositionEmbeddingRandom(nn.Module):
    ...
    def _pe_encoding(self, coords: torch.Tensor) -> torch.Tensor:
        ...
        # Outputs d_1 x ... x d_n x C_glsa shape
    ...
    def forward(self, size: Tuple[int, int]) -> torch.Tensor:
        ...
        return pe.permute(2, 0, 1)  # C_glsa x H x W
    ...
    def forward_with_coords(self, coords_input: torch.Tensor, image_size: Tuple[int, int]) -> torch.Tensor:
        ...
        return self._pe_encoding(coords.to(torch.float))  # B x N x C_glsa
...
class REAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        B, H, W, _ = x.shape
        # qkv with shape (3, B, nHead, H * W, C_glsa)
        qkv = self.qkv(x).reshape(B, H * W, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4)
        # q, k, v with shape (B * nHead, H * W, C_glsa)
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.proj(x).permute(0, 2, 3, 1)  # B C_glsa H W -> B H W C_glsa
