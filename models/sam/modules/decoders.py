...
class SAM2MaskDecoder(nn.Module):
    ...
    def forward(
        ...
        Args:
            image_embeddings (torch.Tensor): Embeddings from the image encoder with shape (B, C_glsa, H, W).
            image_pe (torch.Tensor): Positional encoding with the shape of image_embeddings (B, C_glsa, H, W).
            sparse_prompt_embeddings (torch.Tensor): Embeddings of the points and boxes with shape (B, N, C_glsa).
            dense_prompt_embeddings (torch.Tensor): Embeddings of the mask inputs with shape (B, C_glsa, H, W).
            multimask_output (bool): Whether to return multiple masks or a single mask.
        ...
        Returns:
            (Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]): A tuple containing:
                ...
                - sam_tokens_out (torch.Tensor): Batched SAM token for mask output with shape (B, N, C_glsa).
